...
class CodeGraphBuilder:
    ...
    def _add_node(
        ...
        if node_id not in self.seen_nodes:
            self.graph.add_node(
                node_id, type=node_type, name=name, file=file, **(metadata or {})
            )
            self.seen_nodes.add(node_id)

    def _add_edge(
        self, source: str, target: str, edge_type: str, metadata: dict | None = None
    ):
        ...
        for file_path in root.glob(pattern):
            if file_path.is_file():
                ...
                if files_scanned % 10 == 0:
                    print(
                        f"Scanned {files_scanned} files, extracted {entities_extracted} entities"
                    )
    ...
    def find_related_files(
        self, file_path: str, max_distance: int = 2
    ) -> list[tuple[str, int]]:
        """
        ...
        return [(self.graph.nodes[fid]["file"], dist) for fid, dist in related.items()]
