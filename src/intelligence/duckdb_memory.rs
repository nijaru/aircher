...
impl DuckDBMemory {
    ...
    pub async fn record_pattern(&self, pattern: Pattern) -> Result<()> {
        ...
        db.execute(
            "INSERT INTO patterns (id, description, context, embedding_text, success, 
        ...
        // Update similarities with recent patterns
        self.update_similarities(&db, &pattern).await?;
        
        Ok(())
    }
    
    async fn update_similarities(&self, db: &DuckDBConnection, pattern: &Pattern) -> Result<()> {
