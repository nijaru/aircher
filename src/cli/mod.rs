...
impl CliApp {
    ...
    pub async fn run(&mut self, args: Vec<String>) -> Result<()> {
        ...
        match message {
            ...
            None => {
                // Interactive mode
                self.handle_interactive(&matches).await
            }
        }
    }
    ...
    async fn handle_one_shot(&mut self, message: &str, matches: &clap::ArgMatches) -> Result<()> {
        ...
        info!("One-shot request: provider={}, model={}", provider_name, model);
        ...
        let provider = providers.get_provider_or_host(provider_name)
            .ok_or_else(|| anyhow::anyhow!("Provider '{}' not found or not configured", provider_name))?;
    ...
    async fn handle_interactive(&mut self, _matches: &clap::ArgMatches) -> Result<()> {
        println!("🚧 Interactive mode not yet implemented");
        println!("   For now, use: aircher \"your message here\"");
        println!("   Example: aircher \"Hello, how are you?\"");
        Ok(())
    }
