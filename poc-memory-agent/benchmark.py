...
class AgentSimulator:
    ...
    def find_relevant_files(self, task: BenchmarkTask) -> Tuple[List[str], BenchmarkResult]:
        """
        ...
        if self.use_memory:
            ...
            all_files = (task.target_files +
                        task.related_files +
                        random.sample(task.irrelevant_files,
                                    min(5, len(task.irrelevant_files))))
...
def run_benchmark() -> Dict:
    ...
    def aggregate(results: List[BenchmarkResult]) -> Dict:
        return {
            ...
            'avg_correct_files': sum(r.correct_files_found for r in results) / len(results),
            'avg_irrelevant': sum(r.irrelevant_files_examined for r in results) / len(results),
    ...
    tool_call_reduction = (baseline_stats['avg_tool_calls'] -
                          memory_stats['avg_tool_calls']) / baseline_stats['avg_tool_calls']

    file_reduction = (baseline_stats['avg_files_examined'] -
                     memory_stats['avg_files_examined']) / baseline_stats['avg_files_examined']

    irrelevant_reduction = (baseline_stats['avg_irrelevant'] -
                           memory_stats['avg_irrelevant']) / max(baseline_stats['avg_irrelevant'], 0.1)
    ...
    print(f"Success rate: {memory_stats['success_rate'] - baseline_stats['success_rate']:+.0%}")
